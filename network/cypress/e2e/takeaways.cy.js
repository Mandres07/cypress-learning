...
describe('Takeaways', () => {
  ...
  it('should add a new takeaway', () => {
    cy.intercept('POST', '/takeaways/new*', 'success').as('subscribe');
    cy.login();
    cy.visit('/takeaways/new');
    cy.get('[data-cy="title"]').click();
    cy.get('[data-cy="title"]').type('TestTitle1');
    cy.get('[data-cy="body"]').type('ThisIsTheDescription');
    cy.get('[data-cy="create-takeaway"]').click();
    cy.wait('@subscribe').its('request.body').should('match', /TestTitle1.*ThisIsTheDescription/);
  });
});