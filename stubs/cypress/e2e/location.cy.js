...
describe('share location', () => {
  beforeEach(() => {
    ...
    cy.visit('/').then(window => {  // -> using then after vy.visit() gets the window object
      ...
      cy.stub(window.navigator.clipboard, 'writeText').as('saveToClipboard')
        .resolves();
  ...
  it('should share a location URL', () => {
    ...
    cy.get('@userLocation').then(fakePosition => {
      ...
      cy.get('@saveToClipboard').should(
        ...
        new RegExp(`${latitude}.*${longitude}.*${encodeURI(fakePosition.userName)}`)
      );
